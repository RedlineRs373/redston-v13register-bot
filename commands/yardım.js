...
exports.run = async (client, message, args) => {
  ...
  const yt = new MessageButton().setLabel('RedSton Creative Youtube').setStyle('LINK').setURL('https://youtube.com/UCdaJ0CECNiIo5puJny4Htdw');
  const dc = new MessageButton().setLabel('Xertz Discord').setStyle('LINK').setURL('https://discord.gg/vWXyBy4JRS');
  const row = new MessageActionRow().addComponents(dc, yt)
  const embed = new MessageEmbed()
  .setTitle(`Xertz v13 Kayıt Botu`)
